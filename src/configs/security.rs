...
impl JwtConfig {
  pub fn from_env()-> anyhow::Result<Self>{
    ...
    let expires_in_minutes: i64 = std::env::var("JWT_EXPIRES_IN").ok().and_then(|s| s.parse::<i64>().ok()).unwrap_or(60*24);
...
pub fn sign_jwt(cfg: &JwtConfig, user_id:i64, name:&str, email:&str, role:&str) -> anyhow::Result<String> {
  let now = Utc::now();
